...
{       /* number of bytes of data (up to 501) */
    ...
    len = MSTP_Create_Frame((uint8_t *) & mstp_port->OutputBuffer[0],   /* where frame is loaded */
...
case MSTP_RECEIVE_STATE_IDLE:
        ...
        if (mstp_port->DataAvailable == true) {
            ...
            fprintf(stderr, "MSTP Rx %03u: %02X ",
                (unsigned) timestamp_ms(), mstp_port->DataRegister);
#else
...
CRC_Calc_Header(mstp_port->DataRegister,
    mstp_port->HeaderCRC);
mstp_port->FrameType = mstp_port->DataRegister; mstp_port->Index = 1;}
            /* Destination */
else
...
CRC_Calc_Header(mstp_port->DataRegister,
mstp_port->HeaderCRC);
mstp_port->DestinationAddress = mstp_port->DataRegister; mstp_port->Index = 2;}
            /* Source */
else
...
CRC_Calc_Header(mstp_port->DataRegister,
mstp_port->HeaderCRC);
mstp_port->SourceAddress = mstp_port->DataRegister; mstp_port->Index = 3;}
            /* Length1 */
else
...
CRC_Calc_Header(mstp_port->DataRegister,
mstp_port->HeaderCRC);
mstp_port->DataLength = mstp_port->DataRegister * 256; mstp_port->Index = 4;}
            /* Length2 */
else
...
CRC_Calc_Header(mstp_port->DataRegister,
mstp_port->HeaderCRC);
mstp_port->DataLength += mstp_port->DataRegister; mstp_port->Index = 5;}
            /* HeaderCRC */
else
if (mstp_port->Index == 5) {
mstp_port->HeaderCRC =
CRC_Calc_Header(mstp_port->DataRegister,
mstp_port->HeaderCRC); mstp_port->HeaderCRCActual = mstp_port->DataRegister;
...
mstp_port->receive_state = MSTP_RECEIVE_STATE_IDLE;}
                    ...
                    /* Note: proposed change to BACnet MSTP state machine!
                       ...
                       is part of the data. */
                    /* Data */
if ((mstp_port->DataLength) && (mstp_port->DataLength <= mstp_port->InputBufferSize)) {
...
mstp_port->DataRegister;
mstp_port->Index++; mstp_port->receive_state = MSTP_RECEIVE_STATE_DATA;}
            /* CRC1 */
else
if (mstp_port->Index == mstp_port->DataLength) {
mstp_port->DataCRC = CRC_Calc_Data(mstp_port->DataRegister,
...
mstp_port->DataCRC = CRC_Calc_Data(mstp_port->DataRegister,
mstp_port->DataCRC); mstp_port->DataCRCActualLSB = mstp_port->DataRegister;
#if PRINT_ENABLED_RECEIVE_DATA
fprintf(stderr, "%s", mstptext_frame_type(mstp_port->FrameType));
#endif
...
default:
            /* SendNoWait */
        mstp_port->master_state = MSTP_MASTER_STATE_DONE_WITH_TOKEN; break;}
    }
    break;
...
mstp_port->master_state = MSTP_MASTER_STATE_IDLE; transition_now = true;}
}
else {
...
mstp_port->master_state = MSTP_MASTER_STATE_USE_TOKEN; transition_now = true;}
else {
...
MSTP_Create_And_Send_Frame(mstp_port,
    ...
    if (next_poll_station != mstp_port->This_Station) {
        ...
        MSTP_Create_And_Send_Frame(mstp_port,
            FRAME_TYPE_POLL_FOR_MASTER,
            mstp_port->Poll_Station,
            mstp_port->This_Station, NULL, 0); mstp_port->RetryCount = 0;
...
length); mstp_port->master_state = MSTP_MASTER_STATE_IDLE;}
else {
            /* DeferredReply */
...
/* Return: amount of PDU data */
    ...
    void testReceiveNodeFSM(Test * pTest) {
        ...
        buffer[7] = 0x00;
        Load_Input_Buffer(buffer, len); for (i = 0; i < len; i++) {
