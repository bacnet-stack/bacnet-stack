...
LOG_MODULE_REGISTER(net_websocket, LOG_LEVEL_DBG);
...
static struct websocket_context contexts[BSC_CLIENT_WEBSOCKETS_MAX_NUM];
		      ...
		      int32_t timeout, void *user_data)
{
	...
	if ((ret >=0) && (errno == EAGAIN)) {
		NET_DBG("[%p] EAGAIN happend for host %s", ctx, wreq->host);
		ret = -EAGAIN;
		// save sec_accept_key into tail of tmp_buf for persisining
		p = ctx->tmp_buf + ctx->tmp_buf_len - WS_SHA1_OUTPUT_LEN,
		memcpy(p, sec_accept_key, WS_SHA1_OUTPUT_LEN);
		// ... and memories it in context
		ctx->sec_accept_key = p;
		goto again;
	}
		      ...
		      void *user_data)
{
	...
	ctx = websocket_find(sock);
	if (!ctx) {
		NET_DBG("[%p] Websocket cannot find context for socket %d", ctx, sock);
		return -ENOENT;
	}

	// emulate HTTP request for parsing responce
	req.method = HTTP_GET;
	req.url = wreq->url;
	req.host = wreq->host;
	req.protocol = "HTTP/1.1";
	...
	req.http_cb = &http_parser_settings;
	req.recv_buf = wreq->tmp_buf;
	req.recv_buf_len = wreq->tmp_buf_len - WS_SHA1_OUTPUT_LEN;
	...
	ret = http_wait_data_asyn(sock, &req, ctx);
	if (ret == -EAGAIN) {
		NET_DBG("[%p] EAGAIN happend", ctx);
		goto again;
	}

	if (ret < 0) {
		NET_DBG("[%p] Cannot connect to Websocket host (%d)", ctx, ret);
		ret = -ECONNABORTED;
		goto out;
	}
		       ...
		       uint32_t *message_type, uint64_t *remaining, int32_t timeout)
{
	...
	if (!ctx->header_received) {
		...
		ret = input_len;
#else
		ret = zsock_recv(ctx->real_sock, &ctx->tmp_buf[ctx->tmp_buf_pos],
	...
	if (ctx->tmp_buf_pos == 0) {
		...
		ret = input_len;
#else
		ret = zsock_recv(ctx->real_sock, ctx->tmp_buf, ctx->tmp_buf_len,
			   K_TIMEOUT_EQ(tout, K_NO_WAIT) ? ZSOCK_MSG_DONTWAIT : 0);
#endif /* CONFIG_NET_TEST */
...
static int bsc_websocket_init(const struct device *unused)
{
    ARG_UNUSED(unused);
...
SYS_INIT(bsc_websocket_init, APPLICATION, CONFIG_BSC_WEBSOCKET_APP_PRIORITY);
