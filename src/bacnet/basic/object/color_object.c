...
static const int32_t Writable_Properties[] = {
    ...
    PROP_PRESENT_VALUE, PROP_COLOR_COMMAND,
    PROP_DEFAULT_COLOR, PROP_DEFAULT_FADE_TIME,
    PROP_TRANSITION,    PROP_OBJECT_NAME,
    PROP_DESCRIPTION,   -1
};
...
/**
 * @brief Get the list of writable properties for a Color object
 ...
 * @return true if object-name was set.
 */
static bool Color_Object_Name_Write(
 ...
 * @return true if description was set.
 */
static bool Color_Description_Write(
    BACNET_WRITE_PROPERTY_DATA *wp_data, BACNET_CHARACTER_STRING *cstring)
{
    bool status = false;
    struct object_data *pObject;
...
bool Color_Write_Property(BACNET_WRITE_PROPERTY_DATA *wp_data)
{
    ...
    switch (wp_data->object_property) {
        case PROP_OBJECT_NAME:
            ...
            if (status) {
                status = Color_Object_Name_Write(
        ...
        case PROP_DESCRIPTION:
            status = write_property_type_valid(
                wp_data, &value, BACNET_APPLICATION_TAG_CHARACTER_STRING);
            if (status) {
                status = Color_Description_Write(
                    wp_data, &value.type.Character_String);
            }
            break;
        case PROP_PRESENT_VALUE:
