    ...
    switch (wp_data->object_property) {
        ...
        case PROP_RECIPIENT_LIST:
            ...
            for (idx = 0; idx < NC_MAX_RECIPIENTS; idx++) {
                ...
                uint8_t ft_hundredths = TmpNotify.Recipient_List[idx].FromTime.hundredths;
                ...
                uint8_t tt_hundredths = TmpNotify.Recipient_List[idx].ToTime.hundredths;

                if ((ft_hour > 23 || ft_min > 59 || ft_sec > 59 || ft_hundredths > 100) ||
                    (tt_hour > 23 || tt_min > 59 || tt_sec > 59 || tt_hundredths > 100)) {
                    wp_data->error_class = ERROR_CLASS_PROPERTY;
                    wp_data->error_code = ERROR_CODE_VALUE_OUT_OF_RANGE;
                    status = false;
                    break;
