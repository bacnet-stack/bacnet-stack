...
#define BACNET_CLIENT_WEBSOCKET_TX_BUFFER_SIZE  4096
#define BACNET_SERVER_WEBSOCKETS_MAX_NUM        4
#define BACNET_WSURL_MAX_LEN                    256

typedef int BACNET_WS_HANDLE;
...
typedef enum {
  BACNET_WEBSOCKET_HUB_CONECTION = 0,
  BACNET_WEBSOCKET_DIRECT_CONECTION = 1
...
typedef struct BACNetWebsocketClient {
 ...
 /**
  * @brief Blocking bws_сonnect() function opens a new connection with specified websocket server.
  *        In a case if some other thread calls bws_disconnect() or bws_deinit() when the function
  *        call is in progress, one of following retcodes is returned:
  *           BACNET_WEBSOCKET_NOT_INITIALIZED
  ...
  *          BACNET_WEBSOCKET_BAD_PARAM if a user passed some incorrect parameters.
  *          BACNET_WEBSOCKET_NO_RESOURCES if client has already opened more than
  *                                        BACNET_CLIENT_WEBSOCKETS_MAX_NUM sockets,
  *                                        or if some mem allocations failed or
  *                                        some allocations of resources 
  *                                        failed like mutex, thread, or etc..
  *
  * @param type - type of BACNet/SC connection, check BACNET_WEBSOCKET_CONNECTION_TYPE enum.
  *               According BACNet standard different type of connections require different 
  *               websocket protocols.
  * @param url - BACNet/SC server URL.
  * @param out_handle - pointer to opened websocket handle in a case of successfull connect.
