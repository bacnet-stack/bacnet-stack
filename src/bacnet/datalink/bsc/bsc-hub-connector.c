    ...
    BVLC_SC_DECODED_MESSAGE *decoded_pdu)
{
    ...
    DEBUG_PRINTF("hub_connector_socket_event() >>> hub_connector = %p, socket "
                 ...
                 "pdu = %p, pdu_len = %d\n",
        hc, c, ev, reason, reason_desc, pdu, pdu_len);
    ...
    if (ev == BSC_SOCKET_EVENT_CONNECTED) {
        if (hc->state == BSC_HUB_CONNECTOR_STATE_CONNECTING_PRIMARY) {
            ...
            hub_conector_update_status(&hc->primary_status, BACNET_CONNECTED,
                ERROR_CODE_SUCCESS, NULL);
            ...
            hub_conector_update_status(&hc->failover_status, BACNET_CONNECTED,
                ERROR_CODE_SUCCESS, NULL);
        ...
        if (reason == ERROR_CODE_NODE_DUPLICATE_VMAC) {
            ...
            if (hc->state == BSC_HUB_CONNECTOR_STATE_CONNECTED_PRIMARY) {
                hub_conector_update_status(
                    &hc->primary_status, st, reason, reason_desc);
            } else {
                hub_conector_update_status(
                    &hc->failover_status, st, reason, reason_desc);
            }
            DEBUG_PRINTF(
    ...
    BSC_HUB_CONNECTOR_HANDLE *h)
{
    ...
    ret = bsc_init_ctx(&c->ctx, &c->cfg, &bsc_hub_connector_ctx_funcs, c->sock,
