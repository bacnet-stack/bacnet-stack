    ...
    volatile struct mstp_port_struct_t *mstp_port)
{
    ...
    printf_receive
        ...
        mstp_port->SilenceTimer(mstp_port->UserData1));
    switch (mstp_port->receive_state) {
        ...
        case MSTP_RECEIVE_STATE_IDLE:
            ...
            if (mstp_port->ReceiveError == true) {
                ...
                mstp_port->SilenceTimerReset(mstp_port->UserData1);
...
/*                    mstp_port->StartTs = ((sysTimestampLong()/1000)*121)/1000;*/
        ...
        case MSTP_RECEIVE_STATE_PREAMBLE:
            /* Timeout */
            if (mstp_port->SilenceTimer(mstp_port->UserData1) > Tframe_abort) {
        ...
        case MSTP_RECEIVE_STATE_HEADER:
            ...
            if (mstp_port->SilenceTimer(mstp_port->UserData1) > Tframe_abort) {
                ...
                printf_receive_error("MSTP: Rx Header: SilenceTimer %u > %d\n",
                    (unsigned) mstp_port->SilenceTimer(mstp_port->UserData1), Tframe_abort);
            }
            /* Error */
            else if (mstp_port->ReceiveError == true) {
                mstp_port->ReceiveError = false;
    ...
    volatile struct mstp_port_struct_t * mstp_port)
{
    ...
    if (mstp_port->master_state != master_state) {
        ...
        printf_master
            ...
            mstp_port->TokenCount, mstp_port->SilenceTimer(mstp_port->UserData1),
    ...
    switch (mstp_port->master_state) {
        ...
        case MSTP_MASTER_STATE_IDLE:
            ...
            /* LostToken */
            if (mstp_port->SilenceTimer(mstp_port->UserData1) >= Tno_token) {
        ...
        case MSTP_MASTER_STATE_WAIT_FOR_REPLY:
            ...
            if (mstp_port->SilenceTimer(mstp_port->UserData1) >= Treply_timeout) {
                /* ReplyTimeout */
        ...
        case MSTP_MASTER_STATE_PASS_TOKEN:
            ...
            if (mstp_port->SilenceTimer(mstp_port->UserData1) <= Tusage_timeout) {
        ...
        case MSTP_MASTER_STATE_NO_TOKEN:
            ...
            if (mstp_port->SilenceTimer(mstp_port->UserData1) < my_timeout) {
                if (mstp_port->EventCount > Nmin_octets) {
                    /* SawFrame */
                ...
                if (mstp_port->SilenceTimer(mstp_port->UserData1) < ns_timeout) {
                    /* GenerateToken */
        ...
        case MSTP_MASTER_STATE_POLL_FOR_MASTER:
            ...
            if (mstp_port->ReceivedValidFrame == true) {
                ...
                mstp_port->ReceivedValidFrame = false;
            } else if ((mstp_port->SilenceTimer(mstp_port->UserData1) > Tusage_timeout) ||
                (mstp_port->ReceivedInvalidFrame == true)) {
                if (mstp_port->SoleMaster == true) {
                    /* SoleMaster */
