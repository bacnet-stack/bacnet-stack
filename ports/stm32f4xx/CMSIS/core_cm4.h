...
{
  /* enable interrupt */
  NVIC->ISER[(((uint32_t)(int32_t)IRQn) >> 5UL) & 0x7UL] = (uint32_t)(1UL << (((uint32_t)(int32_t)IRQn) & 0x1FUL));
}
...
{
  NVIC->ICER[((uint32_t)(IRQn) >> 5UL)] = (uint32_t)(1UL << ((uint32_t)(IRQn) & 0x1FUL)); /* disable interrupt */
}
...
static __INLINE uint32_t NVIC_GetActive(IRQn_Type IRQn)
{
  ...
  return((uint32_t)((NVIC->IABR[(uint32_t)(IRQn) >> 5UL] & (1UL << ((uint32_t)(IRQn) & 0x1FUL)))?1UL:0UL));
}
