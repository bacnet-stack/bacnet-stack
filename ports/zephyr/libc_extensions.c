...
FILE *libc_ext_fopen(const char *filename, const char *mode)
{
	...
	if (mode != NULL) {
		for (const char *pc = mode; *pc != 0; pc++) {
			if (*pc == 'r') flags |= FS_O_READ;
			if (*pc == 'w') flags |= FS_O_WRITE;
			if (*pc == 'a') flags |= FS_O_APPEND;
		}
		if (flags & FS_O_WRITE)
			flags |= FS_O_CREATE;
	}
...
size_t libc_ext_fwrite(const void *ZRESTRICT ptr, size_t size, size_t nitems,
...
size_t libc_ext_fread(void *ZRESTRICT ptr, size_t size, size_t nitems,
	      FILE *ZRESTRICT pFile)
{
