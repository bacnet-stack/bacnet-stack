...
static struct nvs_fs *audit_log_fs = NULL;
#endif
...
{
    // first MAX_AUDIT_LOGS number are LogInfo indexes
    return MAX_AUDIT_LOGS + index * AL_MAX_ENTRIES + number;
}

static bool GetLog(AL_LOG_REC *rec, unsigned index, unsigned number)
{
    if (index >= MAX_AUDIT_LOGS || number >= AL_MAX_ENTRIES ||
        audit_log_fs == NULL) {
        return false;
    }
    if (0 >= nvs_read(audit_log_fs, log_id(index, number), rec,
        sizeof(AL_LOG_REC))) {
...
static bool SetLog(AL_LOG_REC *rec, unsigned index, unsigned number)
{
    if (index >= MAX_AUDIT_LOGS || number >= AL_MAX_ENTRIES ||
        audit_log_fs == NULL) {
        return false;
    ...
    return 0 < nvs_write(
        audit_log_fs, log_id(index, number), rec, sizeof(AL_LOG_REC));
}

static void LogInfo_Init(AL_LOG_INFO *rec)
{
    rec->bEnable = false;
    rec->out_of_service = false;
    rec->ulRecordCount = AL_MAX_ENTRIES;
    rec->ulTotalRecordCount = 0;
    rec->iIndex = 0;
}

static bool GetLogInfo(AL_LOG_INFO *rec, unsigned index)
{
    if (index >= MAX_AUDIT_LOGS || audit_log_fs == NULL) {
        return false;
    }
    if (0 >= nvs_read(audit_log_fs, index, rec, sizeof(AL_LOG_INFO))) {
        LogInfo_Init(rec);
    }
    return true;
}

static bool SetLogInfo(AL_LOG_INFO *rec, unsigned index)
{
    if (index >= MAX_AUDIT_LOGS || audit_log_fs == NULL) {
        return false;
    }
    return 0 < nvs_write(audit_log_fs, index, rec, sizeof(AL_LOG_INFO));
}
    ...
    uint32_t instance, BACNET_LOG_STATUS eStatus, bool bState)
{
    ...
    SetLog(&TempRec, log_index, CurrentLog.iIndex++);
...
static int AL_Rec_Search(int iLog, int max_index, AL_LOG_REC *rec)
{
    int i;
    AL_LOG_REC r;
...
void AL_Insert_Notification_Rec(uint32_t instance, AL_NOTIFICATION *notif)
{
    ...
    index = AL_Rec_Search(iLog, CurrentLog.iIndex, &TempRec);
    if (index >= 0) {
        SetLog(&TempRec, iLog, index);
    } else {
...
void AL_Enable_Set(uint32_t instance, bool bEnable)
{
    ...
    if (log_index < MAX_AUDIT_LOGS) {
        GetLogInfo(&CurrentLog, log_index);
        if (CurrentLog.bEnable != bEnable) {
            CurrentLog.bEnable = bEnable;
            SetLogInfo(&CurrentLog, log_index);
        }
    }
}
...
bool AL_Out_Of_Service(uint32_t instance)
{
    ...
    GetLogInfo(&CurrentLog, log_index);
    return CurrentLog.out_of_service;
}
...
void AL_Out_Of_Service_Set(uint32_t instance, bool b)
{
    ...
    if (log_index < MAX_AUDIT_LOGS) {
        GetLogInfo(&CurrentLog, log_index);
        if (CurrentLog.out_of_service != b) {
            CurrentLog.out_of_service = b;
            SetLogInfo(&CurrentLog, log_index);
        }
    }
}
...
int AL_encode_by_time(uint8_t *apdu, BACNET_READ_RANGE_DATA *pRequest)
{
    ...
    if (pRequest->Count < 0) {
        ...
        for (;;) {
            GetLog(&rec, pRequest->object_instance, 
                (uiIndex + iCount) % AL_MAX_ENTRIES);
            if (rec.tTimeStamp < tRefTime) {
                break;
            }
...
int AL_encode_entry(uint8_t *apdu, int iLog, int iEntry)
{
    ...
    GetLog(&pSource, iLog, (iEntry - 1) % AL_MAX_ENTRIES);
    ...
    switch (pSource.ucRecType) {
        ...
        case AL_TYPE_NOTIFICATION:
            ...
            iLen += encode_context_unsigned(&apdu[iLen], 4,
                pSource.Datum.notification.operation);
