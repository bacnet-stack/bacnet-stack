...
static pthread_mutex_t bsc_runloop_mutex = PTHREAD_RECURSIVE_MUTEX_INITIALIZER;
...
static void* bsc_runloop_worker(void *arg)
{
  ...
  while(1) {
    ...
    to.tv_nsec = to.tv_nsec + (BSC_DEFAULT_RUNLOOP_TIMEOUT_MS % 1000) * 1000000;
    to.tv_sec += to.tv_nsec / 1000000000;
    to.tv_nsec %= 1000000000;

    while (!bsc_runloop_process) {
          if (pthread_cond_timedwait(&bsc_cond, &bsc_runloop_mutex, &to) == ETIMEDOUT) {
             break;
          }
    }
    ...
    if(!bsc_runloop_started) {
      ...
      for(i=0; i<BSC_MAX_CONTEXTS_NUM; i++) {
        if(bsc_runloop_ctx[i].ctx) {
          bsc_runloop_ctx[i].runloop_func(bsc_runloop_ctx[i].ctx);
        }
      }
    }
  }
...
{
  int i;
  debug_printf("bsc_runloop_reg() >>> ctx = %p, func = %p\n", ctx, runloop_func);
  ...
  if(bsc_runloop_started) {
    for(i=0; i < BSC_MAX_CONTEXTS_NUM; i++) {
      if(!bsc_runloop_ctx[i].ctx) {
        ...
        pthread_mutex_unlock(&bsc_runloop_mutex);
        debug_printf("bsc_runloop_schedule() <<< ret = BSC_SC_SUCCESS\n");
        return BSC_SC_SUCCESS;
      }
    }
    pthread_mutex_unlock(&bsc_runloop_mutex);
