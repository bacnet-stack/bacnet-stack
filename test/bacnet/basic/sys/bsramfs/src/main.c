...
/**
 * @brief Unit Test for invalid file start positions in BSRAMFS stream/record
 */
#if defined(CONFIG_ZTEST_NEW_API)
ZTEST(bramfs_tests, test_BSRAMFS_invalid_positions)
#else
...
#endif
{
    ...
    zassert_equal(
        len, 0,
        "read_stream with fileStartPosition=-100 must return 0, got %zu", len);

    /* read_stream with start position exceeding file size must return 0 */
    len = bacfile_sramfs_read_stream_data(
        stream_file.pathname, (int32_t)stream_file.size + 1, read_buf,
        sizeof(read_buf));
    zassert_equal(
        len, 0,
        "read_stream with position beyond file size must return 0, got %zu",
        len);
...
void test_main(void)
{
    ztest_test_suite(
        ...
        ztest_unit_test(test_BSRAMFS_invalid_positions));
