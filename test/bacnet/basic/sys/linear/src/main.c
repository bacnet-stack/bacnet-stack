...
void testLinearInterpolateRound(void)
{
    ...
    y2 = (uint16_t)linear_interpolate_round(1, 1, 65535, 1, 100);
    zassert_equal(y2, 1, NULL);
    y2 = (uint16_t)linear_interpolate_round(1, 1, 65535, 100, 1);
    zassert_equal(y2, 100, NULL);

    y2 = (uint16_t)linear_interpolate_round(1, 65535, 65535, 1, 100);
    zassert_equal(y2, 100, NULL);
    y2 = (uint16_t)linear_interpolate_round(1, 65535, 65535, 100, 1);
    zassert_equal(y2, 1, NULL);

    y2 = (uint16_t)linear_interpolate_round(1, (65535 / 2), 65535, 1, 100);
    zassert_equal(y2, 50, NULL);

    y2 = (uint16_t)linear_interpolate_round(1, (65535 / 4), 65535, 1, 100);
    zassert_equal(y2, 26, NULL);

    y2 =
        (uint16_t)linear_interpolate_round(1, ((65535 * 3) / 4), 65535, 1, 100);
    zassert_equal(y2, 75, NULL);

    y2 = (uint16_t)linear_interpolate_round(1, 1, 100, 1, 65535);
    zassert_equal(y2, 1, NULL);

    y2 = (uint16_t)linear_interpolate_round(1, 100, 100, 1, 65535);
    zassert_equal(y2, 65535, NULL);

    y2 = (uint16_t)linear_interpolate_round(1, 100 / 2, 100, 1, 65535);
    zassert_equal(y2, 32437, NULL);

    /* scaling from percent to steps and back */
    for (x2 = 1; x2 <= 100; x2++) {
        y2 = (uint16_t)linear_interpolate_round(1, x2, 100, 1, 65535);
        x2_test = (uint16_t)linear_interpolate_round(1, y2, 65535, 1, 100);
        zassert_equal(x2, x2_test, NULL);
    }

    /* test for low-trim, high-trim and scaling from percent to steps */
    for (x2 = 1; x2 <= 100; x2++) {
        y1 = (uint16_t)linear_interpolate_round(1, 20, 100, 1, 65535);
        y3 = (uint16_t)linear_interpolate_round(1, 80, 100, 1, 65535);
        y2 = (uint16_t)linear_interpolate_round(1, x2, 100, y1, y3);
        x2_test = (uint16_t)linear_interpolate_round(y1, y2, y3, 1, 100);
        zassert_equal(x2, x2_test, "x2=%hu x2_test=%hu\n", x2, x2_test);
    }

    y2 = (uint16_t)linear_interpolate_round(1, 1, 65535, 20, 80);
    zassert_equal(y2, 20, NULL);
    y2 = (uint16_t)linear_interpolate_round(1, 1, 65535, 80, 20);
    zassert_equal(y2, 80, NULL);
    y2 = (uint16_t)linear_interpolate_round(1, 65535, 65535, 20, 80);
    zassert_equal(y2, 80, NULL);
    y2 = (uint16_t)linear_interpolate_round(1, 65535, 65535, 80, 20);
    zassert_equal(y2, 20, NULL);
}
