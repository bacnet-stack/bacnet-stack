 ...
 */
static void testCharacterString_Value_name_description_write(void)
{
    uint32_t object_instance = BACNET_MAX_INSTANCE;
    ...
    uint8_t apdu[MAX_APDU] = { 0 };
    const char *test_name = "CSV-NAME-WP";
    const char *test_description = "CharacterString Value description via WP";
    bool status = false;
    int len = 0;

    CharacterString_Value_Cleanup();
    object_instance = CharacterString_Value_Create(BACNET_MAX_INSTANCE);
    zassert_not_equal(object_instance, BACNET_MAX_INSTANCE, NULL);

    wp_data.object_type = OBJECT_CHARACTERSTRING_VALUE;
    ...
    status = CharacterString_Value_Write_Property(&wp_data);
    zassert_true(status, NULL);

    status = CharacterString_Value_Object_Name(object_instance, &object_name);
    zassert_true(status, NULL);
    status = characterstring_ansi_same(&object_name, test_name);
    zassert_true(status, NULL);

    rp_data.object_type = OBJECT_CHARACTERSTRING_VALUE;
    ...
    len = CharacterString_Value_Read_Property(&rp_data);
    zassert_true(len > 0, NULL);
    ...
    status = CharacterString_Value_Write_Property(&wp_data);
    zassert_true(status, NULL);
    zassert_not_null(CharacterString_Value_Description(object_instance), NULL);
    zassert_equal(
        strcmp(
            CharacterString_Value_Description(object_instance),
            test_description),
        0, NULL);
    ...
    status = CharacterString_Value_Delete(object_instance);
    zassert_true(status, NULL);
    CharacterString_Value_Cleanup();
}
/**
...
{
    ztest_test_suite(
        csv_tests, ztest_unit_test(testCharacterString_Value),
        ztest_unit_test(testCharacterString_Value_name_description_write));
