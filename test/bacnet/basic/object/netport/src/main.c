...
static void test_network_port_sc_status_encode_decode(void)
{
    ...
    status = Network_Port_SC_Hub_Function_Connection_Status_Add(
        instance, BACNET_CONNECTED, &ts, &ts, &peer_address, peer_VMAC,
        peer_UUID, ERROR_CODE_VT_SESSION_ALREADY_CLOSED, "hided error message");
    zassert_true(status, NULL);

    status = Network_Port_SC_Hub_Function_Connection_Status_Add(
        instance, BACNET_FAILED_TO_CONNECT, &ts, &ts, &peer_address2, peer_VMAC,
    ...
    status = Network_Port_SC_Primary_Hub_Connection_Status_Set(
        instance, BACNET_DISCONNECTED_WITH_ERRORS, &ts, &ts,
    ...
    status = Network_Port_SC_Failover_Hub_Connection_Status_Set(
        instance, BACNET_FAILED_TO_CONNECT, &ts, &ts,
    ...
    status = Network_Port_SC_Direct_Connect_Connection_Status_Add(
        instance, "connect url", BACNET_FAILED_TO_CONNECT, &ts, &ts,
