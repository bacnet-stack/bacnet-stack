...
static void test_sc_datalink(void)
{
    ...
    netport_object_init(
        SC_DATALINK_INSTANCE, ca_cert, sizeof(ca_cert), server_cert,
        sizeof(server_cert), server_key, sizeof(server_key), NULL, NULL, &uuid1,
        &vmac1, primary_url1, secondary_url1, true, false, false);
...
static void test_sc_datalink_failed_requests(void)
{
    ...
    zassert_equal(
        memcmp(
            r->Peer_UUID.uuid.uuid128, &failed_uuid.uuid[0],
            BVLC_SC_UUID_SIZE) == 0,
        true, NULL);
    bsc_cleanup();
    bsc_node_stop(node2);
