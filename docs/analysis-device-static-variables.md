...
- [x] 현재 per-device 구현 범위와 static 공유 범위를 구분해서 PR 설명에 명시
...
- [x] `Reinitialize_State` / `Reinit_Password`는 `BAC_ROUTING` 조건부 필드로 추가
- [x] Backup/Restore 상태를 `BAC_ROUTING && BACNET_BACKUP_RESTORE` 조건부 중첩 구조체로 추가
- [x] routed read/write에서 Backup property가 per-device helper 경로를 사용
- [x] service gate 변경은 관련 상태 이동 이후 적용
- [x] `BAC_ROUTING` OFF 빌드에서는 기존 static 변수 기반 동작 유지
- [x] timeout countdown 회귀 시나리오 검증

## 구현 반영 메모 (2026-04-28)

계획대로 RD service gate는 virtual device를 통과시키고, state별 지원 여부는 `Device_Reinitialize()`에서 판단한다. Backup/Restore state와 timeout countdown은 routing 모드에서 현재 routed device의 `Backup` 구조체를 사용한다. DCC는 virtual device에서 기존처럼 차단한다.
