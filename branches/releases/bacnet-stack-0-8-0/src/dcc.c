    ...
    BACNET_CHARACTER_STRING * password)
{
    ...
    if (apdu_len_max) {
        ...
        len = bacnet_unsigned_context_decode(&apdu[apdu_len],
            apdu_len_max - apdu_len, 0, &value32);
        if (len > 0) {
            apdu_len += len;
        ...
        if (apdu_len < apdu_len_max) {
            ...
            len = bacnet_enumerated_context_decode(&apdu[apdu_len],
                apdu_len_max-apdu_len, 1, &value32);
            if (len > 0) {
                apdu_len += len;
                if (enable_disable) {
                    *enable_disable =
...
void test_DeviceCommunicationControlMalformedData(
    Test * pTest)
{
    /* payload with enable-disable, and password with wrong characterstring length */
    uint8_t payload_1[] = {0x19, 0x00, 0x2a, 0x00, 0x41};
    /* payload with enable-disable, and password with wrong characterstring length */
    uint8_t payload_2[] = {0x19, 0x00, 0x2d, 0x55, 0x00, 0x66, 0x69, 0x73, 0x74, 0x65, 0x72};
    ...
    len =
        dcc_decode_apdu(&payload_1[0], sizeof(payload_1), &test_invoke_id,
        &test_timeDuration, &test_enable_disable, &test_password);
    ct_test(pTest, len == BACNET_STATUS_ERROR);
    len =
        dcc_decode_apdu(&payload_2[0], sizeof(payload_2), &test_invoke_id,
        &test_timeDuration, &test_enable_disable, &test_password);
    ct_test(pTest, len == BACNET_STATUS_ERROR);
    len =
        dcc_decode_apdu(&payload_3[0], sizeof(payload_3), &test_invoke_id,
        &test_timeDuration, &test_enable_disable, &test_password);
    ct_test(pTest, len == BACNET_STATUS_ERROR);
    len =
    ...
    void)
{
    ...
    assert(rc);
    rc = ct_addTestFunction(pTest, test_DeviceCommunicationControlMalformedData);
    assert(rc);
